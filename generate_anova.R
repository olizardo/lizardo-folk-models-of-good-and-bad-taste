...
ge <- get_primary("good_taste_example_model_min5/document_topic_probabilities.csv", "Good_Ex")
...
res_ge <- run_anova_for_model("Good_Ex")
res_be <- run_anova_for_model("Bad_Ex")
...
combined$Good_Ex <- format_cell(res_ge$F_val, res_ge$p_adj)
combined$Bad_Ex <- format_cell(res_be$F_val, res_be$p_adj)

dir.create("report/Tabs", showWarnings = FALSE)

# Write to LaTeX
latex_str <- "\\begin{table}[htpb]\n\\centering\n\\resizebox{\\textwidth}{!}{\n\\begin{tabular}{lcccc}\n\\toprule\n"
latex_str <- paste0(latex_str, "Domain & Good Taste Def & Bad Taste Def & Good Taste Ex & Bad Taste Ex \\\\\n")
latex_str <- paste0(latex_str, "& $F$ ($p_{adj}$) & $F$ ($p_{adj}$) & $F$ ($p_{adj}$) & $F$ ($p_{adj}$) \\\\\n\\midrule\n")
...
for (i in 1:nrow(combined)) {
  ...
  g_e <- str_replace_all(combined$Good_Ex[i], "\\*\\*\\*", "$^{***}$")
  g_e <- str_replace_all(g_e, "(?<!\\$)\\*\\*(?!\\$)", "$^{**}$")
  g_e <- str_replace_all(g_e, "(?<!\\$)\\*(?!\\$)", "$^{*}$")
  
  b_e <- str_replace_all(combined$Bad_Ex[i], "\\*\\*\\*", "$^{***}$")
  b_e <- str_replace_all(b_e, "(?<!\\$)\\*\\*(?!\\$)", "$^{**}$")
  b_e <- str_replace_all(b_e, "(?<!\\$)\\*(?!\\$)", "$^{*}$")
