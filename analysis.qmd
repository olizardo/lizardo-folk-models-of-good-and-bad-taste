...
## 2.1 Pearson Residual Plots for Schema Combinations
```{r, eval=FALSE}
library(dplyr)
library(ggplot2)
library(readr)
...
# Mapping
topic_names <- list(
    Good_Def = c("-1"="Outlier", "0"="Similarity/Subjectivity", "1"="Aesthetics", "2"="Style/Quality"),
    Bad_Def = c("-1"="Outlier", "0"="Dislike/Subjective", "1"="Poor Choices", "2"="Tacky/Loud", "3"="Low-Brow Media", "4"="Immoral", "5"="Poor Manners"),
    Good_Ex = c("-1"="Outlier", "0"="Fashion/Decor", "1"="Refinement/Art", "2"="Best Friends", "3"="Musical Knowledge", "4"="Film/TV", "5"="Obamas/Class", "6"="Partners/Food"),
    Bad_Ex = c("-1"="Outlier", "0"="Unspecified/Music", "1"="Poor Fashion", "2"="Derivative Media", "3"="Trump/Vulgarity")
)

df <- df %>%
  filter(Good_Def != "-1", Bad_Def != "-1", Good_Ex != "-1", Bad_Ex != "-1") %>%
  mutate(
    Good_Def = topic_names$Good_Def[as.character(Good_Def)],
    Bad_Def = topic_names$Bad_Def[as.character(Bad_Def)],
    Good_Ex = topic_names$Good_Ex[as.character(Good_Ex)],
    Bad_Ex = topic_names$Bad_Ex[as.character(Bad_Ex)]
  )
...
## 3.4 Example Models Domain Distinction Heatmaps
```{r, eval=FALSE}
library(dplyr)
library(tidyr)
library(ggplot2)
...
# Labels
topic_names_g <- c("-1"="Outlier", "0"="Fashion/Decor", "1"="Refinement/Art", "2"="Best Friends", "3"="Musical Knowledge", "4"="Film/TV", "5"="Obamas/Class", "6"="Partners/Food")
topic_names_b <- c("-1"="Outlier", "0"="Unspecified/Music", "1"="Poor Fashion", "2"="Derivative Media", "3"="Trump/Vulgarity")

merged_df <- merged_df %>%
  mutate(
    Good_Ex = topic_names_g[as.character(Good_Ex_Id)],
    Bad_Ex = topic_names_b[as.character(Bad_Ex_Id)]
  ) %>%
  filter(!is.na(Good_Ex) & Good_Ex != "Outlier" & !is.na(Bad_Ex) & Bad_Ex != "Outlier")
...
## 3.5 Predicted Probabilities for Demographics (Archived)
```{r, eval=FALSE}
library(dplyr)
library(tidyr)
library(nnet)
...
good_ex_probs <- read.csv("good_taste_example_model_min5/document_topic_probabilities.csv")
...
# Mapping
topic_names_g <- c("0"="Fashion/Decor", "1"="Refinement/Art", "2"="Best Friends", "3"="Musical Knowledge", "4"="Film/TV", "5"="Obamas/Class", "6"="Partners/Food")
merged_df$Topic <- as.factor(topic_names_g[as.character(merged_df$primary_topic)])
...
## 3.6 PCA of Distinction Domains and Topic ANOVA
```{r, eval=FALSE}
library(dplyr)
library(tidyr)
library(ggplot2)
